...
class AssistantCompletionRequest(ChatCompletionRequest):
    assistant_completion: bool = False
...
def append_system_to_human(messages):
    system_message = ''
    for m in messages:
        if m['role'] == 'system':
        ...
        if m['role'] == 'user':
            m['content'] = system_message + m['content']
            system_message = ''
...
def parse_args():
    ...
    parser.add_argument("--append-system-to-human",
                        action="store_true")
...
async def create_chat_completion(request: AssistantCompletionRequest,
                                 raw_request: Request):
    ...
    if append_sys:
        request.messages = append_system_to_human(request.messages)

    if request.assistant_completion:
...
async def create_logprobs_(request: LogprobsRequest, raw_request: Request):
    request_id = f"prob-{random_uuid()}"

    prompt = request.prompt

    token_ids, error_check_ret = await check_length(request, prompt=prompt)
    prefix_len = len(tokenizer(request.prefix).input_ids)
    ...
    logprobs = []
    for id, logprob in zip(prompt_token_ids, prompt_logprobs):
        if logprob is None:
            logprobs.append(None)
        else:
            logprobs.append(logprob[id])
            
    logprobs = logprobs[prefix_len:]
...
if __name__ == "__main__":
    ...
    append_sys = args.append_system_to_human
