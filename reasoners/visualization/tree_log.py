...
from reasoners.algorithm import MCTSNode, MCTSResult, BeamSearchNode, BeamSearchResult, BFSNode
...
class TreeLog:
    ...
    def from_mcts_results(cls, mcts_results: MCTSResult, node_data_factory: callable = None,
        ...
        def default_edge_data_factory(n: MCTSNode) -> EdgeData:
            if isinstance(n, BFSNode):
                return EdgeData({"Q": n.reward, "action": n.action, **get_reward_details(n)})
            else:
        ...
        if getattr(mcts_results, 'tree_state_after_each_iter', None) is None:
        ...
        for step in range(len(tree_states)):
            ...
            if getattr(mcts_results, 'trace_in_each_iter', None) is not None:
